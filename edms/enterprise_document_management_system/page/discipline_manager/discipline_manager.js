...
frappe.pages["discipline-manager"].on_page_load = function(wrapper){
	...
	wrapper.make_tree = function() {
		var ctype = frappe.get_route()[1] || 'Discipline';
		return frappe.call({
			method: 'edms.enterprise_document_management_system.page.discipline_manager.discipline_manager.get_children',
			args: {ctype: ctype},
			callback: function(r) {
...
frappe.pages['discipline-manager'].on_page_show = function(wrapper){
	// set route
	console.log("page discipline....")
	var ctype = frappe.get_route()[1] || 'Discipline';
	...
	frappe.breadcrumbs.add(frappe.breadcrumbs.last_module || "discipline-manager");
};
...
frappe.TreeChart = Class.extend({
	...
	new_node: function() {
		...
		d.set_primary_action(__("Create New"), function() {
			...
			return frappe.call({
				method: 'edms.enterprise_document_management_system.page.discipline_manager.discipline_manager.add_node',
				args: v,
				callback: function(r) {
					if(!r.exc) {
						d.hide();
